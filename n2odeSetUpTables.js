...
// to connect to database address
const pool = new Pool({
...
// Query to create performance table
...
// Query to create business table
...
// Run given query for table creation
            ...
            fs.createReadStream('./static/TableData/performance_table.csv')
                ...
                .on('data', function (row) {
                    ...
                    pool.query(
                        ...
                        [row['Most Recent Health Inspection More Than 2 Years Ago'], 
                        row['Overall Rating'] || null,
                        row['Health Inspection Rating'] || null, 
                        row['Staffing Rating'] || null, 
                        row['RN Staffing Rating'] || null,
                        row['Total Weighted Health Survey Score'] || null, 
                        row['Number of Facility Reported Incidents'],
                        row['Number of Substantiated Complaints'], 
                        row['Number of Fines'], 
                        row['Total Amount of Fines in Dollars'],
                        row['Number of Payment Denials'], 
                        row['Total Number of Penalties'], 
                        row['performance_id']],
                        (err) => {
                            if (err) {
                                reject(err);
...
// Open CSVs and populate database tables from .csv data
processCSV();
